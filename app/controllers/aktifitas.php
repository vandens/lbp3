...
class Aktifitas extends CI_Controller {
	...
	{  
		...
		$view 				= ($this->_priv->ACTR) ? strtolower('bo/'.__CLASS__.'/index') : 'bo/temp/no_access'; // cek privi READ
	...
	public function getdata(){
		(!$this->session->userdata('user_islogin')) ? redirect('home') : '';
			if($this->_priv->ACTR){
				if(!$this->session->userdata('admin'))
					...
					$columns 	= array(
						...
						array(
							...
							#	$button	.= ($this->_priv->ACTU) ? str_replace('#',base_url(strtolower(__CLASS__)).'/form/'.$d,$this->config->item('update')) : '';
				...
				echo json_encode($this->datatable->simple( $_GET, $this->config->item('db'), 'lbp_t_activity', 'auto', $columns ));	
			}	
	}
