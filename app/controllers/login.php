...
class Login extends CI_Controller {
	...
	{  
		...
		$this->load->library('facebook');
				...
				if($this->input->post()){
					...
					$this->form_validation->set_rules('user_id', 'User Id', 'trim|required|min_length[5]|max_length[50]|xss_clean');
		      ...
		      $d['fb_url'] = $this->facebook->getLoginUrl(array(
                'redirect_uri' => site_url('login/social_check/fb'), 
	...
	public function signin_check($str){
		$post 	= $this->input->post();
		$data 	= $this->user_model->get_user_detail($post['user_id']); #$this->db->get_where('m_user',array('user_id'=>$this->input->post('user_id'),'user_status'=>'active'))->result();
		...
		if($post['word'] != $post['captcha']){
			...
			return false;
		}elseif($data->user_pass != sha1($post['user_id'].md5($post['user_id'].$post['user_pass']))){
