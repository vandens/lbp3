...
class Grup extends CI_Controller {
	...
	public function initiate($data)
	{
		...
		$this->panel_left 	= $this->load->view('bo/panel_left',array('menu'=>'TOO'),true);
	...
	public function confirm()
	{
		...
		$data['sql']	= $this->db->select('modul_id')->from('m_modul')->where('modul_status','active')->order_by('modul_name ASC')->get()->result();
			...
			$this->form_validation->set_rules('group_status', 'Status Grup', 'required|xss_clean');
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					if($data['val'] == 'confirm'){
						...
						$this->session->set_userdata('data_grup',$data);
	...
	public function simpan(){
		...
		$data 				= $this->session->userdata('data_grup');
		$this->session->unset_userdata('data_grup');
		...
		(!$this->session->userdata('user_islogin')) ? redirect('home') : '';
			...
			#		$_GET['columns'][3]['search']['value'] = $this->session->userdata('village_code');
				...
				$columns 	= array(
									...
									array( 'db' => 'group_status', 		'dt' => 2, 'formatter'	=> function($d,$baris){ return $this->config->item('global_'.$d); } ),
				...
				echo json_encode($this->datatable->simple( $_GET, $this->config->item('db'), 'lbp_m_group', 'group_id', $columns ));	
			}	
	}
