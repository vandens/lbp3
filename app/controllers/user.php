...
class User extends CI_Controller {
	...
	public function confirm()
	{
		...
		if($this->_priv->USEC || $this->_priv->USEU){
			if(empty($post['auto']))
			$this->form_validation->set_rules('user_id', 'User ID', 'required|max_length[50]|valid_email|is_unique[m_user.user_id]|xss_clean');
	...
	public function simpan(){
		...
		if(!$data){
			...
			try {
					...
					$data['raw']	= json_encode($this->user_model->get_user_detail($data['auto']));
	...
	public function mailpass(){
		...
		if($sql->num_rows() > 0){
			...
			$data 			= array('[[HEADER]]'		=> $this->load->view('popup/header',array(),true),
									...
									'[[APP_NAME]]'		=> $this->_setting->app_acronim,
			...
			if($ismail == 1){ // kalo postal mail
				...
				$config['useragent']	= $this->_setting->app_acronim;
				$config['mailtype']		= 'html';
				$this->email->initialize($config);					
				$this->email->from($this->_setting->app_noreply,$this->_setting->app_acronim);
	...
	public function delete(){
		...
		if(!$this->_priv->USED){
			...
			try{
				...
				$raw 		= $this->db->where('auto',$key)->get('m_user')->row();
				$sql 		= $this->db->where('auto',$key)->delete('m_user');
				$sql1 		= $this->db->where('user_priv_id',$raw->user_id)->delete('m_priv_user');
		...
		$this->general->writelog('USED',$msg['msg'],json_encode($raw)); 
		...
		(!$this->session->userdata('user_islogin')) ? redirect('home') : '';
			if($this->_priv->USER){
				...
				$columns 	= array(
					...
					array( 'db' => 'pus_code', 		'dt' => 2 ),
					array( 'db' => 'user_isgroup', 	'dt' => 3 ),
				...
				echo json_encode($this->datatable->simple( $_GET, $this->config->item('db'), 'lbp_m_user', 'auto', $columns ));	
			}	
	}
