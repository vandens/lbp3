						...
						<div class="widget-body">
							...
							<div class="space-2"></div>
								<div class="form-group">
									<label class="col-sm-2 control-label" for="form-field-1"> Puskesmas</label>
									<div class="col-sm-10">											
										<select name='pus_code'>
											<option value=""> -- Pilih Salah Satu -- </option>
											<?php										
											foreach($this->_droplist->list_puskes as $v)
											{
												#$sel 	= in_array(strtoupper($v->shop_id),$user_shop) ? 'selected="selected"' : '';
												$sel 	= ($v->pus_code == $pus_code) ? 'selected="selected"' : '';
												echo '<option value="'.$v->pus_code.'_'.$v->pus_name.'" '.$sel.'>'.ucwords(strtolower($v->pus_name)).'</option>';
											}
											?>
								...
								<div class="form-group">
									...
									<div class="col-sm-9">						
										<input type='text' class='col-xs-5 col-sm-6' name='user_id' <?php if($auto) echo 'readonly'; ?> value="<?php echo $user_id; ?>" placeholder='User ID / Email'  required onkeypress='this.value=ignoreSpaces(this.value); ' onkeyup='javascript:this.value = this.value.toLowerCase();'>
									</div>
								</div>
								<div class="space-2"></div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-sm-9">	
										...
										<div id='group' class='col-sm-9 col-xs-6 <?php echo ($user_isgroup == 'No') ? 'hide' : '';?>'>
											<div class='col-sm-8'>	
												<select name='group_id' class='col-xs-5 col-sm-5'>
				...
				<div id='privlist' class="widget-box transparent <?php echo ($user_isgroup == 'No') ? '' : 'hide';?>">
					...
					<div class="widget-body">
						<div class="widget-main" id='content-detail'>	
							<label><input id="checkall" type="checkbox" onclick="if($(this).is(':checked')){$('.checkAll').attr('checked','checked');}else{$('.checkAll').removeAttr('checked');};" value="" name="checkall" data-rel="tooltip">Pilih Semua</label>
							...
							foreach($this->_master_priv as $row => $vals){
								...
								foreach($vals as $rows){
									...
									echo '<label style="width:200px; font-size:12px;"><input type="checkbox" class="checkAll" name="priv['.$rows->priv_code.']" '.$checked.' > '.$rows->priv_desc.'</label>';
								}
							}
							?>							
						</div>
